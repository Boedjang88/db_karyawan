...
export default function PenggajianPage() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [penggajianRes, karyawanRes] = await Promise.all([fetch('/api/penggajian'), fetch('/api/karyawan')]);
      ...
      if (karyawanData.length > 0 && !formData.karyawan_id) setFormData(prev => ({ ...prev, karyawan_id: String(karyawanData[0].id) }));
